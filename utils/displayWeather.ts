...
import weatherDescriptions from '../maps/weather_descriptions.json' with {
  type: 'json',
}
...
export function displayWeather(data: WeatherResponse, location?: any) {
  ...
  console.log(
    ...
    }  ${weatherText.en} / ${weatherText.zh}`,
  )
  console.log(
    `🌍 Location: ${
      location ? location.city : `${data.latitude}, ${data.longitude}`
    } (${data.timezone_abbreviation}, ${data.timezone})`,
  )
  console.log(
    `🌡️  Temperature: ${temp}°C (Feels like ${feelsLike.toFixed(1)}°C)`,
  )
